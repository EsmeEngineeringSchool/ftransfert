...
# des axes X et Y
# ========================================================== 
def numerical_H(puls,G,taus,omegas):
...
# ========================================================== 
def calc_print_infos(taus,puls,verbose=True):
...
def gen_gain(G,taus,puls,intpuls,deriv_integ,classe,gpw_particular):
    ...
    for k,tau in enumerate(taus):
        if tau[1] != 0 :
            string+="\\addplot[line width=2pt,red,dashed,domain=1e"+ \
                str(intpuls[k][0])+":1e"+str(intpuls[k][1])+", samples=51] {"+ \
    ...
    string+="\\addplot[line width=2pt,red,dashed,domain=1e"+ \
             str(intpuls[-1][0])+":1e"+str(intpuls[-1][1])+", samples=51] {"+ \
             str(gain_next)+'+'+str(pente_gain)+"*log10(x)};\n"
...
def gen_phase(taus,puls,intpuls,deriv_integ,classe,gpw_particular):
    ...
    for k,tau in enumerate(taus):
        if tau[1] != 0 :
            ...
            phase+=tau[1]*90.0
            string+="\\addplot[line width=2pt,red,dashed,domain=1e"+ \
                str(intpuls[k][0])+":1e"+str(intpuls[k][1])+", samples=51] {"+str(phase_old)+"};\n"
            string+="\\draw[line width=2pt,red,dashed] (axis cs:1e"+str(intpuls[k][1])+","+str(phase_old)+\
                                                ")  -- (axis cs:1e"+str(intpuls[k][1])+","+str(phase)+");\n"
    string+="\\addplot[line width=2pt,red,dashed,domain=1e"+ \
             str(intpuls[-1][0])+":1e"+str(intpuls[-1][1])+", samples=51] {"+str(phase)+"};\n"
...
# définit par 
# ==================================================================
def write_tex_file(gain,phas,puls,G,taus,gpw_particular=[],fileout="example.tex",verbose=True):

    # ---------------------------
    # Infos relatives au système
    # ---------------------------
    intpuls,w,taus,deriv_integ,classe,m,n = calc_print_infos(taus,puls,verbose)
    # -------------------------
    ...
    f.write("\paragraph{Fonctions réelles du gain et du déphasage}\n")
    f.write(eqn_gain_reel)
    f.write(eqn_gain_reel_dB)
    f.write(eqn_phase_reelle)

    f.write("\paragraph{Quelques valeurs particulières (calculées) :}\n")
    f.write(tab_valeurs_particulieres)

    f.write("\paragraph{Commande pour reproduire ce fichier :}\n")
...
#               ((-90,-20,7),(-90,90,6),(-4,5),1,[(10,-1),(0.1,-1),(0.001,2),(1,0)],"test3.tex")] 
        ...
        for test in tests :
            ...
            print("fichier "+test_filename+" genéré ")
            print()
    ...
    if random_tests :
        ...
        while k<100:
            ...
            while m > n or n == 0 :
                ...
                intpuls,omegas,taus,deriv_integ,classe,m,n = calc_print_infos(taus,puls_axis,verbose=True) 

            test_filename="test_v3_"+str(k)+".tex"
            gain_min,gain_max,phase_min,phase_max,gpw_particular = numerical_H(puls_axis,gain_static,taus,omegas)
    ...
    if False: 
        ...
        print(numerical_H(puls_axis,gain_static,taus))
